...
app.use(
  cors({
    ...
    methods: ["POST", "GET", "DELETE", "PUT", "PATCH", "UPDATE"]
  })
);
...
// Replace existing /genreInsert POST route
app.post("/genreInsert", (req, res) => {
  ...
  const sql0 = `SELECT * FROM person WHERE email = ? AND password = ? AND person_type = ?`;
  const sqlInsert = `INSERT INTO movie_genre (movie_id, genre) VALUES (?, ?) ON DUPLICATE KEY UPDATE genre = VALUES(genre)`;

  db.query(sql0, [email, password, "Admin"], (err, data) => {
    if (err) return res.status(500).json(err);
    if (data.length === 0) return res.status(404).json({ message: "Sorry, You are not Admin!" });

    db.query(sqlInsert, [movieId, genre], (err, result) => {
      if (err) return res.status(500).json(err);
      return res.status(200).json({ message: "Genre added or updated successfully" });
    });
  });
});

// Replace existing /directorInsert POST route
app.post("/directorInsert", (req, res) => {
  ...
  const sql0 = `SELECT * FROM person WHERE email = ? AND password = ? AND person_type = ?`;
  const sqlInsert = `INSERT INTO movie_directors (movie_id, director) VALUES (?, ?) ON DUPLICATE KEY UPDATE director = VALUES(director)`;

  db.query(sql0, [email, password, "Admin"], (err, data) => {
    if (err) return res.status(500).json(err);
    if (data.length === 0) return res.status(404).json({ message: "Sorry, You are not Admin!" });

    db.query(sqlInsert, [movieId, director], (err, result) => {
      if (err) return res.status(500).json(err);
      return res.status(200).json({ message: "Director added or updated successfully" });
    });
  });
});
...
// Fetch all movies with genres and directors
app.get("/movies", (req, res) => {
  const sql = `
    SELECT 
      m.*, 
      GROUP_CONCAT(DISTINCT mg.genre) AS genres, 
      GROUP_CONCAT(DISTINCT md.director) AS directors 
    FROM movie m 
...
app.put("/movies/:id", (req, res) => {
  const { id } = req.params;
  const { name, image_path, language, synopsis, rating, duration, top_cast, release_date } = req.body;

  const sql =
    "UPDATE movie SET name = ?, image_path = ?, language = ?, synopsis = ?, rating = ?, duration = ?, top_cast = ?, release_date = ? WHERE id = ?";

  db.query(
    sql,
    [name, image_path, language, synopsis, rating, duration, top_cast, release_date, id],
    (err, data) => {
      if (err) return res.json(err);
...
// Add PUT route for updating genres (replace existing ones)
app.put("/genreInsert", (req, res) => {
  const email = req.body.email;
  const password = req.body.password;
  const movieId = req.body.movieId;
  const genre = req.body.genre;
  ...
  const sql0 = `SELECT * FROM person WHERE email = ? AND password = ? AND person_type = ?`;
  const sqlDelete = `DELETE FROM movie_genre WHERE movie_id = ?`;
  const sqlInsert = `INSERT INTO movie_genre (movie_id, genre) VALUES (?, ?)`;
  ...
  db.query(sql0, [email, password, "Admin"], (err, data) => {
    ...
    db.query(sqlDelete, [movieId], (err) => {
      if (err) return res.status(500).json(err);
...
// Add PUT route for updating directors (replace existing ones)
app.put("/directorInsert", (req, res) => {
  const email = req.body.email;
  const password = req.body.password;
  const movieId = req.body.movieId;
  const director = req.body.director;
  ...
  const sql0 = `SELECT * FROM person WHERE email = ? AND password = ? AND person_type = ?`;
  const sqlDelete = `DELETE FROM movie_directors WHERE movie_id = ?`;
  const sqlInsert = `INSERT INTO movie_directors (movie_id, director) VALUES (?, ?)`;

  db.query(sql0, [email, password, "Admin"], (err, data) => {
    if (err) return res.status(500).json(err);
    if (data.length === 0) return res.status(404).json({ message: "Sorry, You are not Admin!" });

    // Delete existing directors for this movie
    db.query(sqlDelete, [movieId], (err) => {
      if (err) return res.status(500).json(err);
